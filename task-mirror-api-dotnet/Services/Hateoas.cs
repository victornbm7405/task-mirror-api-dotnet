...
public static class Hateoas
{
    public static IEnumerable<Link> BuildListLinks(string basePath, int page, int pageSize, int total)
    {
        ...
        if (page < totalPages) links.Add(new("next", $"{basePath}?page={page + 1}&pageSize={pageSize}", "GET"));
        if (page > 1) links.Add(new("prev", $"{basePath}?page={page - 1}&pageSize={pageSize}", "GET"));
        return links;
    }
}
